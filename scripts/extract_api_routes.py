...
def capability_route_map(apk_routes: list[ApiRoute]) -> dict[str, dict[str, str]]:
    """Group APK routes by their trailing action segment (the referentiel capability).

    A route's last literal path segment (kebab-case) matches a capability name
    when snake-cased. Routes ending in a path parameter (``{...}``) have no
    action segment and are skipped.
    """
    mapping: dict[str, dict[str, str]] = {}
    for route in apk_routes:
        segment = route.path.rstrip("/").rsplit("/", 1)[-1]
        if not segment or "{" in segment:
            continue
        capability = segment.replace("-", "_")
        mapping.setdefault(capability, {})[route.slug] = route.full_path
...
def write_capability_map(apk_routes: list[ApiRoute], path: Path) -> None:
    ...
    for capability, routes in mapping.items():
        lines.append(f"    {capability!r}: {{")
        for slug, full_path in routes.items():
            lines.append(f"        {slug!r}: {full_path!r},")
        lines.append("    },")
    lines.append("}")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {len(mapping)} capability routes to {path}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--capability-map",
        type=Path,
        help="Regenerate the capability→route Python catalog at this path",
    )
    parser.add_argument(
        "--check",
        action="store_true",
