...
| 🔬 Beta | Lexman solar camera (meari) | settings as `select` / `switch` / `number` (night vision, motion detection, sensitivities, status light, flip, recording duration) + battery, Wi-Fi, charging and SD-card sensors; live view over WebRTC in progress ([#216](https://github.com/cyrilcolinet/enki-integration-hass/issues/216)) |
